...
with st.sidebar:
    ...
    portfolio_val = st.number_input('Portfolio value', value=100000, format="%d")
    
    st.markdown("---")
    calculate_btn = st.button('Calculate VaR', key='calculate_button')

# Main content
st.title('Value at Risk (VaR) Calculator')
st.write("This app calculates VaR using historical, parametric, and Monte Carlo methods.")

# Function to calculate and display VaR
def calculate_and_display_var(tickers, start_date, end_date, rolling_window, confidence_level, portfolio_val):
    ...
    # Layout for charts
    st.subheader("VaR Analysis Charts")
    ...
    with col1:
        ...
        summary_data = {
            "Parameter": ["Tickers", "Start Date", "End Date", "Rolling Window", "Confidence Level", "Portfolio Value"],
            "Value": [', '.join(tickers), start_date, end_date, f"{rolling_window} days", f"{confidence_level:.2%}", f"${portfolio_val:,.2f}"]
        }
    ...
    st.subheader("Previous VaR Calculation Outputs")
    recent_df = pd.DataFrame(st.session_state['recent_outputs'])
    st.table(recent_df)

# Initial run and button click handling
if 'first_run' not in st.session_state:
    st.session_state['first_run'] = True

if st.session_state['first_run'] or calculate_btn:
    st.session_state['first_run'] = False
    calculate_and_display_var(tickers, start_date, end_date, rolling_window, confidence_level, portfolio_val)

# Footer
st.markdown("---")
st.markdown("Created by: Yash | [GitHub](https://github.com/yourusername) | [LinkedIn](https://www.linkedin.com/in/yourprofile)")